...
### 5.3 터미널
```
위는 탭 띠 하나: [＋ 새 터미널] [방 탭(도구 로고 + 폴더 이름 + X)…]. 그 아래 도구 줄(방일 때만), 그 아래 본문.
  새 터미널  카드 하나. 도구 탭 `Codex · Claude · Gemini`(제작사 로고, 카드 머리). 본문은 번호 붙은 네 단계를 2단으로:
             왼쪽 ① 프로젝트 폴더(편집형 콤보박스 = 최근 폴더 + 직접 입력, 아는 프로젝트 드롭다운; 도구 공통)
                  ② 세션: ○ 새 세션 / ○ 기존 세션 이어서(이 폴더·도구의 세션을 인덱스에서 최근순 30개, 고르면 provider.BuildResumeArguments)
             오른쪽 ③ 시작할 때(새 세션만): 프롬프트(없음 | 기본 제공·내 보관함) · 규칙(규칙 파일 유무 + "규칙 편집" → 내 규칙 화면에 폴더를 넘김)
                  ④ 옵션 인자·프리셋(탭별 기억)
             아래 실행 줄: [터미널로 열기](기본, 앱 안) [새 창에서 열기](보조, 설치돼 있을 때만) + 실행될 명령 미리보기. 미설치면 기본 버튼이 "{도구} 설치"
