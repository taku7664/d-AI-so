...
- **내 프롬프트**(별도 대분류): 세션 첫 메시지로 한 번 실행하는 절차형 프롬프트를 기본 제공 + 내 보관함으로 관리한다. 편집기에서 고쳐 저장하고, `프로젝트에 넣기`로 `docs/prompts/{id}.md`에 쓴 뒤 시작 메시지를 복사해 붙인다. 첫 세트: 소규모 프로젝트 기획 인터뷰, 기존 코드베이스 파악
