...
- 여는 방식 두 가지: **내장 터미널**("터미널로 열기", 기본 → 앱 안 xterm 터미널 방이 화면을 채운다) 또는 **외부 터미널**("새 창에서 열기", 보조). 내장은 WebView2가 있어야 하고 없거나 설정이 꺼져 있으면 외부로 폴백한다. 화면은 카드 하나(도구 탭 → 작업 폴더 → 인자 → 실행)와 방 화면 두 상태. 구조·규칙은 ARCHITECTURE §5.3이 정본
