...
- 여는 방식 두 가지: **내장 터미널**("터미널로 열기", 기본 → 앱 안 xterm 터미널 방이 화면을 채운다) 또는 **외부 터미널**("새 창에서 열기", 보조). 내장은 WebView2가 있어야 하고 없거나 설정이 꺼져 있으면 외부로 폴백한다. 화면은 탭 띠([＋ 새 터미널] [열린 방…]) 아래에 새 세션 카드(도구 탭 → 프로젝트 폴더 → 세션 새/기존 → 시작할 때 프롬프트·규칙 → 인자 → 실행) 또는 그 방의 터미널(도구 줄: 찾기·프롬프트·규칙 편집). 구조·규칙은 ARCHITECTURE §5.3이 정본
...
- 스키마 검증 + 오류 위치 표시
- 최근 파일 목록
- 앱 전용 프리셋 라이브러리: `%LOCALAPPDATA%\DAIso\presets\*.daiso` → 프로젝트로 복사/적용
- 기본 제공 프리셋 10개 이상(Git, C++, C#, Python, TypeScript, Rust, Unity, 테스트, 리뷰, 리팩터링, 보안, 문서, API, 한국어 소통)을 앱에 묻어 두고, 규칙 화면의 갤러리에서 미리 본 뒤 그대로 열거나 지금 규칙에 합칠 수 있다
- **내 프롬프트**(별도 대분류): 세션 첫 메시지로 한 번 실행하는 절차형 프롬프트를 기본 제공 + 내 보관함으로 관리한다. 편집기에서 고쳐 저장하고, `프로젝트에 넣기`로 `docs/prompts/{id}.md`에 쓴 뒤 시작 메시지를 복사해 붙인다. 첫 세트 7개: 소규모 프로젝트 기획 인터뷰, 기능 하나 추가 계획, 기존 코드베이스 파악, 버그 재현과 원인 추적, 리팩터링 계획, 배포 전 점검, 작업 회고
