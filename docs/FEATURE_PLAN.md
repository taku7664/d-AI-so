...
- **화면(2026-09-09 재구성, 사용자 피드백)**: 위는 탭 띠 하나 `[＋ 새 터미널] [방…]`. "열린 터미널이 있습니다" 띠는 없앴다. 새 터미널 탭 = 카드 하나: 도구 탭 → 프로젝트 폴더(도구 공통, 아는 프로젝트) → 세션(새 세션 / 기존 세션 이어서 + 그 폴더의 세션 목록) → 시작할 때(프롬프트 선택·규칙 편집; 새 세션만) → 옵션 인자 → 실행(터미널로 열기 기본, 새 창 보조). 프롬프트 선택칸은 작업 폴더에서 빼서 "시작할 때"로 옮겼고, 고르면 docs/prompts에 넣고 시작 메시지를 첫 메시지 인자로 넘긴다. 방 탭 = 터미널이 화면을 채우고 탭 띠 오른쪽에 도구 줄(찾기·프롬프트 넣기·규칙 편집).
- 폴더를 도구별로 나누지 않은 이유: 폴더는 실행 위치일 뿐이라 충돌이 없고, 나누면 같은 폴더를 세 번 고르게 된다.
...
**뒤로**
- ~~`/` 선택기~~ **구현** — `SlashCommandReader`(내장 표 + `~/.claude/commands`·프로젝트 `.claude/commands`·`~/.claude/skills`, Codex `~/.codex/prompts`, Gemini `~/.gemini/commands/*.toml`, 단위 테스트 4건)와 방 입력의 AutoSuggestBox 선택기. 라이브 드롭다운은 사람이 확인
- Claude B 프로토콜(stream-json, 스트리밍·승인 카드)
- ~~터미널 안 검색~~ **구현**(xterm search 애드온 + 방 머리 찾기 칸), ~~글꼴 크기 설정~~ **구현**(설정), ~~턴 종료 배지~~ **구현**(다른 탭에 답이 오면 탭에 점), ~~터미널 탭의 내 프롬프트 드롭다운~~ **구현**(작업 폴더에 적용 여부 표시 + 클릭해 적용)
...
- **"실행 중" 판정 완료** — 세션 목록의 실행 중 배지가 도구 표식뿐 아니라 `RoomManager.IsRunning`(앱 안 방이 그 세션 파일을 tail 중)도 본다. 목록을 다시 읽을 때 반영된다.

**첫 판 범위 전부 구현 완료.** 남은 것은 계획에서 '뒤로 미룬 것'뿐: `/` 스킬 선택기, Claude B 프로토콜(글자 단위 스트리밍·승인 카드), 터미널 안 검색, 글꼴 크기 설정. 그리고 사람이 확인할 라이브 항목: 자식 CLI 한 턴 → 채팅 블록.
